...
patryk-wojciechowicz-009'''.split(),
    '桌游': '''ardhira-putra-008'''.split(),
    '电影': '''aleksey-rico-001
aleksey-rico-002
aleksey-rico-003
aleksey-rico-006
aleksey-rico-009'''.split(),
    '生日': [],
    '杂项': '''lam-002
lam-007
magoyama-001
...
expected = {'派对': 3, '运动': 1, '外出': 9, '桌游': 1, '电影': 5, '生日': 0, '杂项': 6}
assert len(catalog_ids) == 93, len(catalog_ids)
assert len(submitted) == 93, len(submitted)
assert not [cover_id for cover_id, count in counts.items() if count > 1]
assert set(submitted) == set(catalog_ids), (set(submitted) - set(catalog_ids), set(catalog_ids) - set(submitted))
assert {category: len(ids) for category, ids in categories.items()} == expected
assert len(deprecated) == 68
