...
def test_weather_invalidation_reuses_a_pending_snapshot_without_autoflush(db_session, admin_user) -> None:
    activity = make_activity(db_session, admin_user)
    assert db_session.autoflush is False
    first = ensure_weather_snapshot(db_session, activity, now=NOW)
    activity.start_time += timedelta(days=1)
    invalidated = invalidate_weather_snapshot(db_session, activity, now=NOW)
    assert invalidated is first
    db_session.commit()
    snapshots = db_session.query(ActivityWeatherSnapshot).all()
    assert len(snapshots) == 1
    assert snapshots[0].target_date == activity.start_time.date()
