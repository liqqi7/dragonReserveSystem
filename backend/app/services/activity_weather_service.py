...
def ensure_weather_snapshot(db: Session, activity: Activity, *, now: datetime | None = None) -> ActivityWeatherSnapshot:
    ...
    now = now or app_now()
    # With autoflush=False, a newly added snapshot is not in the identity map
    # yet. Repeated normalization in this transaction must reuse that pending row.
    snapshot = next((row for row in db.new if isinstance(row, ActivityWeatherSnapshot)
                     and row.activity_id == activity.id), None)
    if snapshot is None:
